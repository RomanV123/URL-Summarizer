...
class URLSummarizer:
    def __init__(self):
        ...
        # Load keywords from JSON file in the script directory
        keywords_path = os.path.join(self.base_path, 'keywords.json')
...
Respond ONLY with valid JSON. No markdown, no explanation, just the JSON object."""
    ...
    def process_url_list(self, urls):
        ...
        output_file = os.path.join(self.base_path, 'article_summaries.xlsx')
    ...
    def add_single_url(self, url):
        """Add a single URL to the Excel sheet"""
        output_file = os.path.join(self.base_path, 'article_summaries.xlsx')
        
        print(f"\n{'='*60}")
        print(f"Adding single URL to {output_file}")
        print(f"{'='*60}\n")
        ...
        if os.path.exists(output_file):
            ...
            if url in existing_df['URL'].values:
                print(f"⚠️  This URL already exists in the Excel sheet!")
                print(f"{'='*60}\n")
                return
        ...
        if status == 'Success':
            print(f"\n{'='*60}")
            print(f"✓ Successfully added to {output_file}")
...
def main():
    ...
    if '--limit' in sys.argv:
        try:
            ...
            print(f"✓ Testing mode: Limited to {limit} URLs\n")
        except (IndexError, ValueError):
            print("ERROR: Invalid --limit value. Usage: python main.py --limit 5\n")
            return
    ...
    print(f"\n{'='*60}")
    print("SELECT MODE:")
    print("  1. Batch process all URLs from urls.txt")
    print("  2. Add a single URL")
    print("="*60)
    ...
    if choice == '2':
        # Single URL mode
        single_url = input("\nEnter the URL to add: ").strip()
        if not single_url:
            print("ERROR: No URL provided!")
            return
        
        try:
            summarizer.add_single_url(single_url)
        ...
        # Batch mode (default)
        # Read URLs from the script directory
            ...
            with open(urls_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if 'http://' in line or 'https://' in line:
                        line = line[line.find('http'):].strip()
                    urls.append(line)
        ...
        if limit:
            urls = urls[:limit]
            print(f"✓ Processing {len(urls)} URL(s) [LIMITED]\n")
        else:
            print()
    ...
    print("Done! Check your Excel file (article_summaries.xlsx) for results!\n")
